...
# Index tensors are int by nature — pick the right ttnn dtype per torch dtype.
_TORCH_TO_TTNN_INDEX_DTYPE = {
...
def _run_gather(
    ...
    # bf16 native is bit-exact; composite paths bounce through TILE so use PCC.
...
def test_gather_tile(input_shape, index_shape, dim, input_factory, index_factory, output_factory, dtype, device):
    _run_gather(
        ...
        dtype,
        device,
        ulp_when_exact=(dtype == ttnn.bfloat16),
    )
...
def test_gather_irregular_shapes_interleaved(input_shape, index_shape, layout, device):
    _run_gather(
        ...
        ttnn.bfloat16,
        device,
        ulp_when_exact=(layout == ttnn.TILE_LAYOUT),
    )
...
@pytest.mark.parametrize(
    "input_shape, index_shape, element_layout, input_factory, index_factory, out_mem_layout, ulp_when_exact",
    [
...
def test_gather_col_major_orientation_preserved(
    input_shape, index_shape, element_layout, input_factory, index_factory, out_mem_layout, ulp_when_exact, device
):
