...
class Qwen36Model:
    ...
    def warmup_prefill_masked_buckets(self, page_table, buckets=None):
        """Compile all masked-bucket programs before trace capture (prevents compile-clobber hang)."""
        if buckets is None:
            buckets = self._PREFILL_MASK_BUCKETS
        block_size = get_block_size(self._paged_kv_caches)
        # Warm every fill width w=1..max_width (paged_fill_cache keys on ceil(valid_len/64)).
