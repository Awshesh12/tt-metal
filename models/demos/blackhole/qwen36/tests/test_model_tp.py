...
vLLM contract path (paged KV + decode) must match bespoke prefill_tp/decode_tp (B=1 oracle).
8 layers for speed. Tests: short contract, long eager/traced prefill, batched decode/prefill.
...
def test_model_tp_contract(mesh_device, reset_seeds, ensure_gc):
    ...
    # Per-step logits PCC (teacher-forced; avoids argmax-tie compounding).
    pccs = []
    ...
    # Masked fixed-bucket prefill (T=128 routes entirely through masked bucket).
...
def test_model_tp_long_prefill_traced(mesh_device, T, reset_seeds, ensure_gc):
    """Captured chunk-outer trace prefill must match bespoke single-pass (T=4096 or 4352)."""
    nd = mesh_device.get_num_devices()
    assert nd > 1, "this test exercises the TP (num_devices>1) contract path"
    ...
    # Bespoke oracle (run before trace capture).
    model.reset_tp()
    ref = model.prefill_tp(torch.tensor([prompt], dtype=torch.long), valid_len=T).reshape(-1).float()

    # Capture chunk-outer trace, then replay per chunk + tail.
    block_size = 64
    num_blocks = math.ceil(((T // block_size) + 8) / 32) * 32  # 32-aligned for flexible SDPA
    page_table = torch.arange(num_blocks, dtype=torch.int32).reshape(1, num_blocks)
    kv_shape = (num_blocks, args.n_local_kv_heads, block_size, args.head_dim)
    model.allocate_kv_caches(kv_shape, ttnn.bfloat16, batch_size=1)
...
def test_model_tp_decode_batched(mesh_device, B, reset_seeds, ensure_gc):
    """Batched paged decode must match B independent B=1 bespoke runs (per-user PCC)."""
    import gc
    ...
    # Batched: per-user paged prefill + diverging-position decode.
...
def test_model_tp_prefill_traced_bucket(mesh_device, B, reset_seeds, ensure_gc, request):
    """Traced bucket prefill must match eager prefill_paged_peruser (prefill logits, GDN state, decode)."""
    import gc
    ...
    # Exactly bucket=128 prompts only (traced path; sub-bucket uses eager).
    bucket = 128
    prompt_lens = [bucket] * B
    block_size = 64
    bpu = max(4, -(-(bucket + N_DEC + 4) // block_size))
    ...
    # Eager reference (prefill_paged_peruser).
    ...
    # Snapshot eager GDN rec_state per user.
    eager_gdn = [
    ...
    # Per-user prefill + GDN state + decode PCC (traced vs eager).
...
def test_model_tp_prefill_chunked_batched(mesh_device, B, seqlen, reset_seeds, ensure_gc, request):
    """prefill_chunked_peruser must match B=1 prefill_traced_chunked (isl2048/4096/mixed lengths)."""
    import gc
    ...
    bpu = ((bpu + 7) // 8) * 8  # SDPA page-table stick alignment
    ...
    # B=1 reference: prefill_traced_chunked + decode per user.
    ...
    for u in range(B):
        ...
        oracle_rec.append(
            [
                ttnn.to_torch(la.attention.rec_state, mesh_composer=comp0)[0].float()
    ...
    # Batched: prefill_chunked_peruser + decode.
    ...
    # Per-user prefill + GDN state + decode PCC (batched vs B=1).
